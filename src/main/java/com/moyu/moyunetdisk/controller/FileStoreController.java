...
import com.moyu.moyunetdisk.utils.FtpUtil;
import com.moyu.moyunetdisk.utils.MailUtils;
import com.moyu.moyunetdisk.utils.QRCodeUtil;
...
public class FileStoreController extends BaseController {
    ...
    public Map<String, Object> uploadFile(@RequestParam("file") MultipartFile files) {
        ...
        try {
            ...
            if (b){
                //上传成功
                log.info("文件上传成功!"+files.getOriginalFilename());
                //向数据库文件表写入数据
        ...
        if (myFile != null){
            ...
            if (!oldName.equals(newName)){
                ...
                if (b){
                    ...
                    if (integer == 1){
                        log.info("修改文件名成功!原文件名:"+oldName+"  新文件名:"+newName);
                    }else{
                        log.error("修改文件名失败!原文件名:"+oldName+"  新文件名:"+newName);
                    }
                }
            }
        }
